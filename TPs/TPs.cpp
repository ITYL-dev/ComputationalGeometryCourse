...
public:
    ...
    Vertex3(std::initializer_list<double> position, int index_adjacent_triangle, bool is_virtual = false) :
    ...
    void readOFF(const char* filename) {
        ...
        while (std::getline(ifs, line)) { // parsing using order (comments not supported)
            ...
            if (line_counter == 1) { // 2nd line (assuming there is a first line with OFF)
                ...
                for (int i{ 0 }; i < nb_vertices; i++) addVertex(Vertex3({ 0, 0, 0 }, 0));
                ...
                edges.reserve(3);
                edges.emplace_back(std::set<int>{ triangles[i].vertices_indices[1], triangles[i].vertices_indices[2] }); // relative index 0
                edges.emplace_back(std::set<int>{ triangles[i].vertices_indices[2], triangles[i].vertices_indices[0] }); // relative index 1
                edges.emplace_back(std::set<int>{ triangles[i].vertices_indices[0], triangles[i].vertices_indices[1] }); // relative index 2
